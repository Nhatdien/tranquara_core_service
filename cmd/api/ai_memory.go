...
// internalGetUserJournalsHandler returns recent journals for a specific user.
// GET /v1/internal/user-journals?user_id=xxx&since=2026-03-04T00:00:00Z
func (app *application) internalGetUserJournalsHandler(w http.ResponseWriter, r *http.Request) {
	userIDStr := r.URL.Query().Get("user_id")
	sinceStr := r.URL.Query().Get("since")

	if userIDStr == "" || sinceStr == "" {
		app.errorResponse(w, r, http.StatusBadRequest, "Missing 'user_id' or 'since' query parameter")
		return
	}
	...
	journals, err := app.models.UserJournal.GetAllSince(userID, since)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}

	// Return simplified journal data for memory extraction
	...
	summaries := make([]journalSummary, len(journals))
	...
	err = app.writeJson(w, http.StatusOK, envolope{"journals": summaries}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
}

// internalGetUserMemoriesHandler returns existing memories for a specific user.
// GET /v1/internal/ai-memories/:user_id
func (app *application) internalGetUserMemoriesHandler(w http.ResponseWriter, r *http.Request) {
	params := httprouter.ParamsFromContext(r.Context())
	...
	memories, err := app.models.AIMemory.GetAllByUser(userID, "")
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}

	err = app.writeJson(w, http.StatusOK, envolope{"memories": memories}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
}
